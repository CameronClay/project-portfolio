...
export async function PATCH(request: NextRequest) {
    ...
    const { data, response } = await parse_params_resp(request, params.update_user as Param[]);
    if (response !== null) {
        return response;
    }
    ...
    const result = await users_db.update_user(jwt_info.user_id, new_username, new_password ? generate(new_password) : null);
