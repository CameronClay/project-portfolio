...
export async function on_request_error(response: Response | null, error: unknown) {
    ...
    const status_code = (response !== null && response.status !== null) ? response.status : 500;
...
export async function parse_params(request: NextRequest, param_info: Param[]): Promise<Record<string, unknown>> {
...
export async function parse_params_resp(request: NextRequest, param_info: Param[]): Promise<{ data: Record<string, unknown>, response: NextResponse | null }> {
